...
#SBATCH --output=/rna/liha/phylogenomics_practice/QC-DB/Scripts/logs/REFSEQ_%j.log
#SBATCH --error=/rna/liha/phylogenomics_practice/QC-DB/Scripts/logs/REFSEQ_%j.log
#
# 00-Check-is-in-refseq-protein.sh
#
# Adapted from TENT/TENT2/Scripts/02-Curated-species.sh for QC-DB's own
# species_list.tsv. Produces species_info.tsv, the master table every
# downstream QC-DB script (01-Check_pooled_counts.sh, 02-Select-assemblies.sh)
# reads.
#
# For each requested species in species_list.tsv:
#   1. Resolve the name to an NCBI taxid via local names.dmp (any name
#      class, so synonyms resolve too).
#   2. Query the local refseq_protein BLAST db for that taxid AND all of
#      its descendants (strains/subspecies) -- taxid expansion is
#      intentional here.
#   3. Count protein records and accession-prefix breakdown (NP_/XP_/WP_/
#      YP_/OTHER) per returned taxid.
#   4. Select the single most protein-rich descendant taxid per requested
#      species (ties: prefer the root taxid itself, then the numerically
#      smaller taxid). This one-descendant selection is deliberate --
#      downstream QC-DB steps (01/02/03) are built to operate on exactly
#      one taxid per species, not a taxon-exploded total.
#   5. Write the final summary table.
#
# Usage: sbatch 00-Check-is-in-refseq-protein.sh
#   (or: bash 00-Check-is-in-refseq-protein.sh, for an interactive/manual run)

set -euo pipefail

SCRIPT_DIR=${CHECK_REFSEQ_WORK_DIR:-/rna/liha/phylogenomics_practice/QC-DB/Scripts}
...
SPECIES_LIST=${SPECIES_LIST:-/rna/liha/phylogenomics_practice/QC-DB/species_list.tsv}
